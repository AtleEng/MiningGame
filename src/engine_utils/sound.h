...
// ################################     Sound Functions   ################################ 
void sound_play(char* soundName, SoundOptions options = 0)
{
	LOG_ASSERT(soundName, "No Sound name supplied!");
	...
	if(wavFile)
	{
		...
		{
			LOG_ERROR("Exausted Sounds Buffer!\nCapacity:\t%d\nBytes Used:\t%d\nSound Path:\t%s\nSound Size:\t%d",
...
void sound_stop(char* soundName)
{
	sound_play(soundName, SOUND_OPTION_FADE_OUT);
}
